...
impl Transform2D {
    ...
    pub fn get_flip_v(&self) -> Option<&str> {
        self.flip_v.get_value_unchecked()
    }
    ...
    pub fn get_flip_h(&self) -> Option<&str> {
        self.flip_h.get_value_unchecked()
    }
    ...
    pub(crate) fn write_to(&self, writer: &mut Writer<Cursor<Vec<u8>>>) {
        ...
        if let Some(v) = self.rot.get_value_unchecked() {
            attributes.push(("rot", v).into());
        }
        if let Some(v) = self.flip_h.get_value_unchecked() {
            attributes.push(("flipH", v).into());
        }
        if let Some(v) = self.flip_v.get_value_unchecked() {
