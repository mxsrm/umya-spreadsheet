...
impl TextFontType {
    ...
    fn write_to(&self, writer: &mut Writer<Cursor<Vec<u8>>>, tab_name: &str) {
        ...
        if self.typeface.has_value() {
            attributes.push(("typeface", self.typeface.get_value_string()).into());
        }
        if self.pitch_family.has_value() {
            attributes.push(("pitchFamily", self.pitch_family.get_value_string()).into());
        }
        if self.charset.has_value() {
            attributes.push(("charset", self.charset.get_value_string()).into());
        }
        if self.panose.has_value() {
            attributes.push(("panose", self.panose.get_value_string()).into());
        }
