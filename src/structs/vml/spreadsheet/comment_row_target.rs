...
impl CommentRowTarget {
    ...
    pub fn get_value(&self) -> u32 {
        self.value.get_value_unchecked()
    }
    ...
    pub(crate) fn adjustment_insert_row(&mut self, num_rows: u32) {
        let value = self.value.get_value_unchecked() + num_rows;
        self.value.set_value(value);
    }
    ...
    pub(crate) fn adjustment_remove_row(&mut self, num_row: u32) {
        if self.value.get_value_unchecked() > num_row {
            let value = self.value.get_value_unchecked() - num_row;
            self.value.set_value(value);
        } else {
...
impl AdjustmentValue for CommentRowTarget {
    ...
    fn adjustment_insert_value(&mut self, root_num: u32, offset_num: u32) {
        self.value.set_value(
            adjustment_insert_coordinate(
                self.value.get_value_unchecked() + 1,
                root_num,
                offset_num,
            ) - 1,
        );
    ...
    fn is_remove_value(&self, root_num: u32, offset_num: u32) -> bool {
        is_remove_coordinate(self.value.get_value_unchecked() + 1, root_num, offset_num)
    }
}
