...
impl PivotCacheDefinition {
    ...
    pub fn get_refreshed_date(&self) -> f64 {
        self.refreshed_date.get_value_unchecked()
    }
    ...
    pub fn get_refreshed_version(&self) -> u8 {
        self.refreshed_version.get_value_unchecked()
    }
    ...
    pub fn get_record_count(&self) -> u32 {
        self.record_count.get_value_unchecked()
    }
    ...
    pub(crate) fn write_to(&self, writer: &mut Writer<Cursor<Vec<u8>>>) {
        ...
        if self.id.has_value() {
            attributes.push(("r:id", self.id.get_value_string()).into());
        }
        if self.refreshed_by.has_value() {
            attributes.push(("refreshedBy", self.refreshed_by.get_value_string()).into());
        }
